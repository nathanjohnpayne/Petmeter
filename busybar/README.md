...
What that saves is JSON, and nothing else. Measured on the bar, `JSON.stringify` takes 650–850 ms over a sixteen-element sweep and 140–230 ms over a four-element card, while the POST takes 0.4–0.7 s at either size.

**The tombstones were never most of the delay — `for...of` was.** On the bar's JerryScript an allocation costs ~10 ms, and `for...of` allocates on every step: walking the eighteen ids took 340 ms, against 11 ms with an indexed loop. The id bookkeeping and the text-width helpers were all `for...of`, so indexing them took a four-element frame from 1–1.3 s to build down to 0.3–0.6 s, and the time from a parsed reply to the draw returning from 1.7–2.2 s down to 1.1–1.4 s. Raising the app's heap to 128 KiB moves none of these numbers: the cost is the runtime's, and the only fix is asking it for less.
